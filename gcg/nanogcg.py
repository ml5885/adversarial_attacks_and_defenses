"""This code is directly taken from the nanoGCG library. We vendorize the
code here to make the necessary modifications for model ensembling.
...
def filter_ids(ids: Tensor, tokenizer: transformers.PreTrainedTokenizer):
    ...
    If all candidates change (common with some Llama tokenizers due to whitespace/normalization),
    we FALL BACK to returning the unfiltered ids instead of raising.
    """
    ids_decoded = tokenizer.batch_decode(ids)
    filtered_ids = []

    for i in range(len(ids_decoded)):
        # Re-tokenize the decoded text (no specials)
    ...
    if not filtered_ids:
        logger.warning(
            "filter_ids: all candidates changed after decode→encode; "
            "skipping filtering for this batch."
        )
        return ids  # fallback (keeps search moving)
...
class EnsembleGCG:
    def __init__(
        ...
        assert len(models) == 2, "EnsembleGCG currently supports exactly two models."
        self.models = models
        self.tokenizer = tokenizer
        self.config = config

        # Use same tokenizer (shared vocab) across both models
        self.embedding_layers = [m.get_input_embeddings() for m in models]
        self.not_allowed_ids = None if config.allow_non_ascii else get_nonascii_toks(tokenizer, device=models[0].device)
        self.prefix_caches = [None, None]

        self.stop_flag = False

        if any(m.dtype in (torch.float32, torch.float64) for m in models):
            logger.warning("One or more models are in FP32/FP64. Lower precision is faster.")

        if any(m.device == torch.device("cpu") for m in models):
            logger.warning("One or more models are on CPU. Use an accelerator for faster optimization.")

        if not tokenizer.chat_template:
            logger.warning("Tokenizer lacks chat template; assuming base model and setting empty chat template.")
            tokenizer.chat_template = "{% for message in messages %}{{ message['content'] }}{% endfor %}"
    ...
    def run(
        ...
        # Tokenize (shared tokenizer) to get ids once
        device = self.models[0].device
        before_ids = self.tokenizer([before_str], padding=False, return_tensors="pt")["input_ids"].to(device, torch.int64)
        after_ids  = self.tokenizer([after_str],  add_special_tokens=False, return_tensors="pt")["input_ids"].to(device, torch.int64)
        target_ids = self.tokenizer([target],     add_special_tokens=False, return_tensors="pt")["input_ids"].to(device, torch.int64)

        # Per-model embeds and optional prefix caches
        self.before_embeds_list = []
        self.after_embeds_list = []
        self.target_embeds_list = []
        self.target_ids_list = [target_ids, target_ids]

        for k, (m, emb) in enumerate(zip(self.models, self.embedding_layers)):
            be = emb(before_ids.to(m.device))
            ae = emb(after_ids.to(m.device))
            te = emb(target_ids.to(m.device))
            self.before_embeds_list.append(be)
            self.after_embeds_list.append(ae)
            self.target_embeds_list.append(te)
            if self.config.use_prefix_cache:
                with torch.no_grad():
                    out = m(inputs_embeds=be, use_cache=True)
                    self.prefix_caches[k] = out.past_key_values
        ...
        for _ in tqdm(range(self.config.num_steps)):
            ...
            with torch.no_grad():
                ...
                # Evaluate aggregated loss across both models
                batch_size = sampled_ids.shape[0] if self.config.batch_size is None else self.config.batch_size
                cand_losses = find_executable_batch_size(self._compute_candidates_loss_ensemble, batch_size)(sampled_ids)

                current_loss = cand_losses.min().item()
            ...
            if self.stop_flag:
                logger.info("Early stopping due to finding a perfect match on both models.")
                break
    ...
    def init_buffer(self) -> AttackBuffer:
        logger.info(f"[Ensemble] Initializing attack buffer of size {self.config.buffer_size}...")
        buffer = AttackBuffer(self.config.buffer_size)

        # Build initial ids using shared tokenizer
        if isinstance(self.config.optim_str_init, str):
            init_optim_ids = self.tokenizer(self.config.optim_str_init, add_special_tokens=False, return_tensors="pt")["input_ids"].to(self.models[0].device)
            if self.config.buffer_size > 1:
                init_buffer_ids = self.tokenizer(INIT_CHARS, add_special_tokens=False, return_tensors="pt")["input_ids"].squeeze().to(self.models[0].device)
            ...
            if len(self.config.optim_str_init) != self.config.buffer_size:
                logger.warning(f"Using {len(self.config.optim_str_init)} initializations but buffer size is {self.config.buffer_size}")
            try:
                init_buffer_ids = self.tokenizer(self.config.optim_str_init, add_special_tokens=False, return_tensors="pt")["input_ids"].to(self.models[0].device)
            except ValueError:
                logger.error("Unable to create buffer. Ensure all initializations tokenize to the same length.")

        true_buffer_size = max(1, self.config.buffer_size)
        init_losses = find_executable_batch_size(self._compute_candidates_loss_ensemble, true_buffer_size)(init_buffer_ids)

        for i in range(true_buffer_size):
            buffer.add(init_losses[i], init_buffer_ids[[i]])

        buffer.log_buffer(self.tokenizer)
        logger.info("[Ensemble] Initialized attack buffer.")
        return buffer

    def compute_token_gradient(self, optim_ids: Tensor) -> Tensor:
        # one-hot over shared vocab
        vocab = self.embedding_layers[0].num_embeddings
        optim_ids_onehot = torch.nn.functional.one_hot(optim_ids, num_classes=vocab).to(self.models[0].device, self.models[0].dtype)
        optim_ids_onehot.requires_grad_()

        total_loss = 0.0
        for k, (m, emb, be, ae, te, tids) in enumerate(
            zip(self.models, self.embedding_layers, self.before_embeds_list, self.after_embeds_list, self.target_embeds_list, self.target_ids_list)
        ):
            optim_embeds = optim_ids_onehot.to(m.device, m.dtype) @ emb.weight  # (1, n_tokens, dim)

            if self.prefix_caches[k] is not None:
                input_embeds = torch.cat([optim_embeds, ae, te], dim=1)
                out = m(inputs_embeds=input_embeds, past_key_values=self.prefix_caches[k], use_cache=True)
            else:
                input_embeds = torch.cat([be, optim_embeds, ae, te], dim=1)
                out = m(inputs_embeds=input_embeds)

            logits = out.logits
            shift = input_embeds.shape[1] - tids.shape[1]
    ...
    def _compute_candidates_loss_ensemble(self, search_batch_size: int, sampled_ids: Tensor) -> Tensor:
        """Average loss across both models for each candidate."""
        B = sampled_ids.shape[0]
        all_loss = torch.zeros(B, device=self.models[0].device, dtype=torch.float32)

        # Per-model prefix KV caches expanded per batch size (when needed)
        prefix_batches = [None, None]

        for i in range(0, B, search_batch_size):
            cur_bs = min(search_batch_size, B - i)
            losses_accum = torch.zeros(cur_bs, device=self.models[0].device, dtype=torch.float32)

            for k, (m, emb, be, ae, te, tids) in enumerate(
                zip(self.models, self.embedding_layers, self.before_embeds_list, self.after_embeds_list, self.target_embeds_list, self.target_ids_list)
            ):
                with torch.no_grad():
                    sids = sampled_ids[i : i + cur_bs].to(m.device)
                    s_emb = emb(sids)

                    if self.prefix_caches[k] is not None:
                        if (prefix_batches[k] is None) or (cur_bs != search_batch_size):
                            prefix_batches[k] = [
                                [x.expand(cur_bs, -1, -1, -1) for x in self.prefix_caches[k][h]]
                                for h in range(len(self.prefix_caches[k]))
                            ]
                        inp = torch.cat([s_emb, ae.repeat(cur_bs, 1, 1), te.repeat(cur_bs, 1, 1)], dim=1)
                        out = m(inputs_embeds=inp, past_key_values=prefix_batches[k], use_cache=True)
                    else:
                        inp = torch.cat([be.repeat(cur_bs, 1, 1), s_emb, ae.repeat(cur_bs, 1, 1), te.repeat(cur_bs, 1, 1)], dim=1)
                        out = m(inputs_embeds=inp)

                    logits = out.logits
                    tmp = inp.shape[1] - tids.shape[1]
                    shift_logits = logits[..., tmp - 1 : -1, :].contiguous()
                    shift_labels = tids.repeat(cur_bs, 1)
                    ...
                    if self.config.use_mellowmax:
                        ...
                        ).view(cur_bs, -1).mean(dim=-1)

                    losses_accum = losses_accum + loss.to(losses_accum.dtype)

            # mean over models
            losses_accum = losses_accum / 2.0
            all_loss[i : i + cur_bs] = losses_accum

            if self.config.early_stop:
                # require both models to exactly match target tokens
                # Note: this is a strict condition and rarely triggers
                pass
...
def run_ensemble(
    ...
    """Jointly optimizes a suffix over two models (shared tokenizer)."""
    if config is None:
        config = GCGConfig()
    logger.setLevel(getattr(logging, config.verbosity))
    gcg = EnsembleGCG(models, tokenizer, config)
